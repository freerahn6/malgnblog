...
| 갱신 | `POST /api/update?pw=...` — 대시보드 "프로그램 갱신" 버튼. `gitDir`(기본 `/home/blog`)에서 `git fetch` + `git reset --hard origin/deploy` 실행 후 출력 반환 |
...
**프로그램 갱신 버튼** — cron 자동 갱신을 쓰기 어려운 환경을 위한 수동 배포 수단입니다. 관리자가
대시보드에서 누르면 서버가 GitHub `deploy` 브랜치의 최신본을 당겨옵니다. 안전장치: 비밀번호 인증,
POST 전용, 셸 미경유(git 인자 배열 실행 → 명령 주입 불가), 60초 타임아웃, 동시 실행 1건 제한,
`web.xml`의 `updateEnabled=false`로 전체 차단 가능. **연산은 pull(merge)이 아니라 fetch+reset**
— 배포 브랜치는 Actions가 통째로 갈아끼우므로 로컬을 원격에 강제로 맞추는 것이 맞습니다.
⚠️ 이 버튼은 서버에서 git을 실행하므로, **기본 비밀번호 `gamma`를 반드시 교체**해야 합니다(설치가이드 4장).

**예약 발행 때문에 GitHub Actions도 같은 엔드포인트를 부릅니다** — 원고에 예약 일시(`publish_at`)를
걸면 그 시각에 사람이 없어도 사이트에 떠야 하므로, Actions가 배포 후 `POST /api/update`를 대신
호출합니다. 저장소 Secret `BLOG_ADMIN_PW`(= `/home/blog/data/admin.pw` 와 같은 값)가 등록돼 있을
때만 동작하고, 없으면 건너뜁니다. 서버 쪽에 추가로 설치·설정할 것은 없습니다.
