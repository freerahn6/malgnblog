...
HOME_EXTRA=('.wrap{max-width:1320px}'
 ...
 '.featured{margin-bottom:28px;padding-bottom:24px;border-bottom:1px solid var(--border)}'
 '.home-shell .grid{grid-template-columns:1fr;gap:0;border-top:1px solid var(--border)}'
 '.card{flex-direction:row;border:none;border-radius:0;border-bottom:1px solid var(--border);box-shadow:none}'
 '.card:hover{transform:none;box-shadow:none}'
 '.card .cover{width:100px;min-width:100px;aspect-ratio:1/1;order:2;border-radius:8px;overflow:hidden}'
 '.card .cover .cat,.card .cover .scrim{display:none}'
 '.card .body{padding:16px 14px 16px 0;order:1;gap:6px}'
 '.card h3{font-size:15.5px;line-height:1.4}'
 '.card .excerpt{display:none}'
 '.card .meta{padding-top:6px;font-size:11.5px}'
 '}'
 +MOBILE_NAV_CSS+NAV_FIT_CSS)
...
--ok:#17795e;--ok-bg:#e2f3ec;--bad:#c4343a;--bad-bg:#fdeaea;}
...
--accent:#5b8cff;--ok:#3ecf8e;--ok-bg:#14301f;--bad:#ff6b6f;--bad-bg:#3a1f22}
...
<script>
/* 이 페이지는 로그인 없이 열리므로 git 실행 로그를 화면에 뿌리지 않는다.
   출력에는 원격 저장소 주소·브랜치·커밋 해시·서버 경로가 그대로 들어 있다.
   성공/실패는 위 문구와 HTTP 상태로만 알린다. 원문 로그가 필요하면 관리자 콘솔에서 본다. */
...
go.addEventListener('click',function(){
  go.disabled=true;say('갱신 중… (최대 1분)','wait');
  fetch('/api/update',{method:'POST'}).then(function(r){
    ...
    else if(x.s===200){say('갱신이 끝까지 진행되지 않았습니다. 서버 담당자 확인이 필요합니다.','bad')}
    ...
    else {say('갱신 실패 (HTTP '+x.s+'). 서버 담당자에게 알려 주세요.','bad')}
  }).catch(function(){
...
# ---- 글감 카탈로그 동봉 — 관리자 콘솔 /gamma2 의 「대기중」 화면이 읽는다 ----
# 마스터 지시: "새 글감을 발굴하면 리스트업하고, 원하지 않는 주제는 삭제할 수 있게.
#              서버 저장하고 내가 삭제할 때까지 영구적으로 저장."
#              (2026-09-23 방향 전환: 공개 /new 페이지 → 비번 뒤의 콘솔 /gamma2#/pending)
#  · 카탈로그(_data/topics.json)는 WEB-INF 아래로 넣는다 — 브라우저가 직접 못 읽는다.
#    콘솔이 비번을 확인한 /api/stats?topics=1 로만 받아 그린다(HTML 에 굽지 않는다).
...
if os.path.isfile(_tsrc):
    try:
        ...
        print(f"[경고] _data/topics.json 을 읽지 못했습니다({_te}) — 콘솔 「대기중」이 빈 목록으로 나갑니다")
else:
    print("[안내] _data/topics.json 없음 — 콘솔 「대기중」이 빈 목록이 된다(파일이 생기면 다음 빌드에 실린다)")
